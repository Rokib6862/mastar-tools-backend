// ============================================================
// MEGA TOOLS — LINK MODEL (v3.0 Production Hardened)
// ============================================================
...
async function create(data) {
  ...
  if (!data.baseUrl && !data.is_chain) {
    throw new Error('Base URL is required for non-chain links');
  }
  if (data.is_chain && (!data.chain_links || data.chain_links.length < 2)) {
    throw new Error('Chain links require at least 2 URLs');
  }

  // Check duplicate code
  const allLinks = await db.links.read();
  if (allLinks.find(l => l.baseCode === code || l.redirectCode === code)) {
...
async function getNextChainUrl(link, visitorId) {
  ...
  // Single DB write for all chain updates
  const updateData = {
  ...
  if (mode === ROTATION_MODES.SEQUENTIAL) {
    updateData.chain_current_index = (selectedIndex + 1) % chainLinks.length;
  }

  await db.links.findByIdAndUpdate(link._id, updateData);
