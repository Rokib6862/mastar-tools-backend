// ============================================================
// MEGA TOOLS — SESSION MODEL (ENTERPRISE v7.1)
// v7.1: User Agent — Full string capture, 1000 char limit, userAgent field
// ============================================================
...
async function create(data) {
  try {
    ...
    const session = {
      ...
      browser: (data.browser || '').substring(0, 1000),
      userAgent: (data.userAgent || data.browser || '').substring(0, 1000),
...
async function upsert(data) {
  try {
    ...
    if (existing) {
      ...
      if (data.browser) {
        existing.browser = data.browser.substring(0, 1000);
        existing.userAgent = data.browser.substring(0, 1000);
      }
      if (data.userAgent) {
        existing.userAgent = data.userAgent.substring(0, 1000);
        if (!existing.browser) existing.browser = data.userAgent.substring(0, 1000);
      }
