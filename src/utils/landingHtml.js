...
// UPDATED v6.5: Real User Agent in Heartbeat + Offline
// ============================================================
...
<body>
  ...
  <script>
    (async function() {
      ...
      // ============================================================
      // HEARTBEAT — Real User Agent included
      // ============================================================
      function startHeartbeat() {
        ...
        heartbeatTimer = setInterval(function() {
          ...
          fetch(API + '/api/data/heartbeat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ visitorId: VID, trackingCode: TK, status: 'Active', browser: navigator.userAgent || '', device: /Mobi/i.test(navigator.userAgent || '') ? 'Mobile' : 'Desktop' })
          }).catch(function() {});
        }, HEARTBEAT_INTERVAL);
      }

      // ============================================================
      // OFFLINE DETECTION — Real User Agent included
      // ============================================================
      function sendOffline() {
        ...
        clearAllTimers();
        cleanupSocket();
        var payload = JSON.stringify({ visitorId: VID, trackingCode: TK, status: 'Offline', browser: navigator.userAgent || '', device: /Mobi/i.test(navigator.userAgent || '') ? 'Mobile' : 'Desktop' });
