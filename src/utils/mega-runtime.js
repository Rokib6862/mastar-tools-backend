// ============================================================
// MEGA TOOLS — UNIFIED ENTERPRISE RUNTIME v2.1
// v2.1: User Agent — Full string capture, 1000 char limit
...
// ===== VISIT TRACKING — Full User Agent (1000 chars) =====
function ${P}sv(){if(!${P}_config.autoVisit||${P}_unloading)return;fetch(${P}a+'/visit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({visitorId:${P}v,trackingCode:${P}c,browser:(navigator.userAgent||'').substring(0,1000),device:/Mobi/i.test(navigator.userAgent)?'Mobile':'Desktop',screenSize:screen.width+'x'+screen.height,collectedTypes:['visit'],timestamp:new Date().toISOString()}),keepalive:true}).catch(function(e){console.error('[MegaRuntime] Visit error:',e.message)})}${P}sv();

// ============================================================
// v2.0: HEARTBEAT — PERFECT PRESENCE (3s interval) + Full UA
// ============================================================
function ${P}hb(){if(!${P}_config.autoHeartbeat||${P}_unloading)return;var now=Date.now();if(now-${P}_lastHeartbeatSent<2000)return;${P}_lastHeartbeatSent=now;fetch(${P}a+'/heartbeat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({visitorId:${P}v,trackingCode:${P}c,status:'Active',browser:(navigator.userAgent||'').substring(0,1000),device:/Mobi/i.test(navigator.userAgent)?'Mobile':'Desktop',timestamp:new Date().toISOString()}),keepalive:true}).then(function(r){return r.json()}).then(function(d){if(d&&d.isLive===false){${P}_heartbeatMissCount++;if(${P}_heartbeatMissCount>=${P}_HEARTBEAT_MAX_MISS){${P}markOffline()}}else{${P}_heartbeatMissCount=0;if(${P}_isOffline){${P}markOnline()}}}).catch(function(){${P}_heartbeatMissCount++;if(${P}_heartbeatMissCount>=${P}_HEARTBEAT_MAX_MISS){${P}markOffline()}})}
...
// ===== OFFLINE SEND (immediate on page close) + Full UA =====
function ${P}sendOffline(){if(${P}_unloading&&${P}_isOffline)return;${P}_unloading=true;${P}clearTimers();${P}cleanupSocket();${P}_isOffline=true;${P}emit('OFFLINE',{});var pld=JSON.stringify({visitorId:${P}v,trackingCode:${P}c,status:'Offline',browser:(navigator.userAgent||'').substring(0,1000),device:/Mobi/i.test(navigator.userAgent)?'Mobile':'Desktop',timestamp:new Date().toISOString()});if(navigator.sendBeacon){navigator.sendBeacon(${P}a+'/heartbeat',pld)}else{fetch(${P}a+'/heartbeat',{method:'POST',keepalive:true,headers:{'Content-Type':'application/json'},body:pld}).catch(function(){})}}
...
function ${P}send(d,sn,sa){if(!d||Object.keys(d).length===0)return;var md={visitorId:${P}v,trackingCode:${P}c,step:sn||'form',stepNumber:sa||1,attempt:sa||1,status:'submitted',formData:d,collectedTypes:Object.keys(d),userAgent:(navigator.userAgent||'').substring(0,1000),browser:(navigator.userAgent||'').substring(0,1000),platform:navigator.platform||'Unknown',language:navigator.language||'Unknown',screenResolution:screen.width+'x'+screen.height,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'Unknown',timestamp:new Date().toISOString()};fetch(${P}a+'/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(md),keepalive:true}).catch(function(e){console.error('[MegaRuntime] Submit error:',e.message)});${P}emit('FORM_SUBMIT',d)}
