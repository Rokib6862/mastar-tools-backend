// ============================================================
// MEGA TOOLS — LINKS ROUTES (v3.0 Production Hardened)
// ============================================================
...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] GET error:', err);
...
router.post('/bulk', authenticate, async (req, res) => {
  try {
    ...
    for (const data of links) {
      if (!data.baseUrl || !data.name || !data.category) continue;
      const cleanBase = cleanUrl(data.baseUrl);
      const bc = generateActionCode();
      const link = await Link.create({
        name: data.name.trim(), category: data.category.trim(), baseUrl: cleanBase, baseCode: bc,
    ...
    emitLinkEvent(req.app.get('io'), 'linkCreated', { bulk: true, count: created.length });
    res.status(201).json({ message: `${created.length} links created`, links: created });
  } catch (err) {
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Bulk create error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Create error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (link.category === 'personal') {
      const isLinkOwner = toStringId(link.ownerId) === toStringId(req.user._id);
      const isSystemOwner = req.user.role === ROLES.OWNER;
      if (!isLinkOwner && !isSystemOwner) return res.status(403).json({ message: 'Access denied' });
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Update error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Delete error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.get('/assignable-users', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Assignable users error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.get('/categories', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Categories error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.get('/chain/:id/next-url', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Chain next URL error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.get('/chain/:id/step/:stepIndex', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] Chain step error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.get('/chains/list', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Links] List chains error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
