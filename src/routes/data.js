// ============================================================
// MEGA TOOLS — DATA TRACKING ROUTES (ENTERPRISE v6.7)
// v6.7: User Agent — Full string capture, 1000 char limit
// ============================================================
...
router.get('/:code', async (req, res) => {
  try {
    ...
    const { session, isNew } = await Session.upsert({ visitorId, baseCode, trackingCode: finalTrackingCode, linkId: link ? toStringId(link._id) : null, ip, browser: ua.substring(0, 1000), deviceType: /mobile/i.test(ua) ? 'Mobile' : 'Desktop', entryUrl: baseCode, currentUrl: code, collectedTypes: ['visit'], chainId: link?.is_chain ? toStringId(link._id) : null, chainStep, chainStepName });
...
router.get('/click/:code', async (req, res) => {
  try {
    ...
    const identity = visitorId || ip;
    if (link?.is_chain) { const chainResult = await getChainTargetUrl(link, identity, actionCode); targetUrl = chainResult.url; chainStep = chainResult.chainStep; chainStepName = chainResult.chainStepName; }
    const { session, isNew } = await Session.upsert({ visitorId, baseCode, trackingCode: finalTrackingCode, linkId: link ? toStringId(link._id) : null, ip, browser: ua.substring(0, 1000), deviceType: /mobile/i.test(ua) ? 'Mobile' : 'Desktop', entryUrl: baseCode, currentUrl: req.params.code, collectedTypes: ['click'], chainId: link?.is_chain ? toStringId(link._id) : null, chainStep, chainStepName });
    sessionManager.updateEvidence(session._id, 'user_activity');
    const io = getIO(); await emitSessionEvents(io, session, isNew, link);
  ...
  try {
    ...
    if (session) {
      ...
      const { session: newSession } = await Session.upsert({
        ...
        baseCode: 'qr_scan',
        ip,
        browser: req.headers['user-agent']?.substring(0, 1000) || '',
