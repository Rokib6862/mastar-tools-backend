// ============================================================
// MEGA TOOLS — THEME ROUTES (v3.0 Production Hardened)
// ============================================================
...
async function getCachedThemes() {
  ...
  const themes = await db.themes.read();
  themeCache = themes;
...
// ============================================================
// THEME STATS (must be before /:code)
// ============================================================
...
router.get('/stats', authenticate, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Theme] Stats error:', err);
...
router.get('/', async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Theme] GET error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.get('/:code', async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Theme] Get single error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});

// ============================================================
// CREATE THEME (Owner only)
// ============================================================
...
router.post('/', authenticate, isOwner, async (req, res) => {
  try {
    ...
    const theme = {
      ...
      createdByName: req.user.name || req.user.username || 'Owner',
    };

    themes.push(theme);
    await db.themes.write(themes);
    clearCache();
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Theme] Create error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.put('/:id', authenticate, isOwner, async (req, res) => {
  try {
    ...
    const updated = {
      ...
      updatedByName: req.user.name || req.user.username || 'Owner',
    };
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Theme] Update error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.delete('/:id', authenticate, isOwner, async (req, res) => {
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Theme] Delete error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});

// ============================================================
// SET DEFAULT THEME (Owner only)
// ============================================================

router.post('/default/:code', authenticate, isOwner, async (req, res) => {
  try {
    const themes = await db.themes.read();
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Theme] Default error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
