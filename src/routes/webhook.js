// ============================================================
// MEGA TOOLS — WEBHOOK ROUTES (ENTERPRISE v4.0)
// FIXED v4.0: db.readJSON → db.webhook_logs.read (MongoDB)
// ============================================================
// LOGIC: visitorId + trackingCode → Session.upsert() Gatekeeper
// ENTERPRISE: Async upsert after response, Room-based emit only
// ENTERPRISE FIX: Added seq field for client-side dedupe
// ============================================================
...
router.post('/click', async (req, res) => {
  ...
  try {
    ...
    // ENTERPRISE: Respond immediately, process async
    ...
    setImmediate(async () => {
      try {
        ...
        console.error('[Webhook] Click async error:', err.message);
      }
    });
  } catch (err) {
    ...
    if (trackingCode) {
      setImmediate(async () => {
        try {
          ...
          console.error('[Webhook] Form submit async error:', err.message);
        }
      });
    }
  } catch (err) {
...
// ============================================================
// LOGS (Owner only — via OWNER_API_TOKEN)
// ============================================================

router.get('/logs', async (req, res) => {
  try {
    const token = (req.headers.authorization || '').split(' ')[1];
    if (token !== CONFIG.OWNER_API_TOKEN) return res.status(403).json({ success: false, message: 'Invalid token' });
    const logs = await db.webhook_logs.read();
