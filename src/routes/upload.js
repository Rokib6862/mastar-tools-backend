// ============================================================
// MEGA TOOLS — IMAGE UPLOAD ROUTES (v3.0 Production Hardened)
// ============================================================
...
router.post('/', optionalAuth, upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Upload] Error:', err.message);
...
router.post('/multiple', optionalAuth, upload.array('images', 5), async (req, res) => {
  ...
  try {
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Upload] Multiple error:', err);
    res.status(500).json({ success: false, message: 'Upload failed' });
  }
});

// ============================================================
// DELETE IMAGE (Query param)
// ============================================================

router.delete('/', optionalAuth, async (req, res) => {
  try {
    const deleteUrl = req.query.deleteUrl || req.query.url || '';
    ...
    if (!CONFIG.IS_PRODUCTION) console.error('[Upload] Delete error:', err);
